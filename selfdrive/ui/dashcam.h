...
static void screen_draw_button(UIState *s, int touch_x, int touch_y) {
  ...
  if (s->vision_connected){
    ...
    int btn_w = 130;
    int btn_h = 130;
    int btn_x = 1920 - btn_w+17;
    int btn_y = 1080 - btn_h+5; //Shift REC button down some -wirelessnet2
    nvgBeginPath(s->vg);
      nvgRoundedRect(s->vg, btn_x-105, btn_y-45+5, btn_w, btn_h, 100); //Shift REC button down some -wirelessnet2
      nvgStrokeColor(s->vg, nvgRGBA(255,255,255,80));
      nvgStrokeWidth(s->vg, 6);
      nvgStroke(s->vg);
      ...
      nvgText(s->vg,btn_x-85,btn_y+40+5,"REC",NULL); //Shift REC button down some -wirelessnet2
  }
