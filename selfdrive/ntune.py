...
class nTune():
  ...
  def checkValidCommon(self):
    ...
    if self.checkValue("steerRatio", 5.0, 25.0, 15.0):
      updated = True

    if self.checkValue("steerActuatorDelay", 0.1, 0.8, 0.125):
      updated = True

    if self.checkValue("steerRateCost", 0.1, 1.5, 0.6):
      updated = True

    if self.checkValue("cameraOffset", -1.0, 1.0, 0.00):
      updated = True
    ...
    try:
      if self.CP is not None:
        ...
        if self.CP.lateralTuning.which() == 'lqr' and self.lqr is not None:
          ...
          self.config["steerActuatorDelay"] = round(self.CP.steerActuatorDelay, 3)
          self.config["steerRateCost"] = round(self.CP.steerRateCost, 3)
